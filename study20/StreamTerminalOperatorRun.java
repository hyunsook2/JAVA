...
public class StreamTerminalOperatorRun {
	public static void main(String[] args) {
		...
		List<String> elements = Arrays.asList("A", "AB", "C", "CD", "E", "F");
		...
		System.out.println(Arrays.stream(arr).anyMatch(num -> num > 8)); //하나라도맞나
		System.out.println(Arrays.stream(arr).allMatch(num -> num < 20)); //다맞았나
		System.out.println(Arrays.stream(arr).noneMatch(num -> num > 20)); //다틀렷나
//collect()
...
//그룹,분할:groupingBy(),partitionBy()
		...
		Double avg2 = nums.stream().collect(Collectors.averagingDouble(num->num)); //평균
		Integer sum2 = nums.stream().collect(Collectors.summingInt(num->num)); //다더하기
		...
		Map<Boolean,List<Integer>> m2 = stream2.collect(Collectors.groupingBy(a->a>100));
